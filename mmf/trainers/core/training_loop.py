...
class TrainerTrainingLoopMixin(ABC):
    ...
    def after_training_loop(self) -> None:
        ...
        if (
            ...
            self.logistics_hook.after_validation(report=report, meter=meter)
        ...
        while self.num_updates < self.max_updates and not should_break:
            ...
            for batch in self.train_loader:
                ...
                if self.num_updates % self.training_config.evaluation_interval == 0:
                    ...
                    stop = self.early_stop_hook.after_validation()
                    self.logistics_hook.after_validation(report=report, meter=meter)
    ...
    def run_train_batch(self, batch: Tensor) -> None:
        report = self._forward(batch)
        loss = self._extract_loss(report)
        self._backward(loss)

        if self.num_updates % self.logistics_hook.log_interval == 0:
            ...
            self.logistics_hook.after_train_step(report=report, meter=self.meter)
    ...
    def _backward(self, loss: Tensor) -> None:
        ...
        self.optimizer.step()
        self.lr_scheduler_hook.after_train_step()
        self.num_updates += 1
