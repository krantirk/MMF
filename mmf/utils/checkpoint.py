...
class Checkpoint:
    ...
    def _load(self, file, force=False, load_pretrained=False):
        ...
        if len(pretrained_state_mapping.items()) == 0:
            ...
            self.trainer.early_stop_hook.early_stopping.init_from_checkpoint(ckpt)
    ...
    def save(self, update, iteration=None, update_best=False):
        ...
        best_iteration = (
            self.trainer.early_stop_hook.early_stopping.best_monitored_iteration
        )
        best_update = self.trainer.early_stop_hook.early_stopping.best_monitored_update
        best_metric = self.trainer.early_stop_hook.early_stopping.best_monitored_value
        model = self.trainer.model
        data_parallel = registry.get("data_parallel") or registry.get("distributed")
